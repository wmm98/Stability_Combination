...
class Eth_Btn_Check_MainWindow(config_path.UIConfigPath):
    ...
    def setupUi(self, MainWindow):
        ...
        self.open_close_btn_label = QLabel("以太网关、开间隔时长(秒)：")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "以太网ping包压测配置界面"))


class EthBtnCheckDisplay(QtWidgets.QMainWindow, Eth_Btn_Check_MainWindow):
    def __init__(self):
        super(EthBtnCheckDisplay, self).__init__()
        self.setupUi(self)
        self.intiui()
    ...
    def handle_submit(self):
        ...
        if len(self.open_close_btn_interval.currentText()) == 0:
            self.get_message_box("请输入4G关、开间隔时长！！！")
            return
        ...
        self.get_message_box("以太网ping包压测配置保存成功")

    def save_config(self):
        config = ConfigP(self.ui_config_file_path)
        section = config.section_eth_check
        config.add_config_section(section)

        # 保存用例压测次数设置
        config.add_config_option(section, config.option_eth_btn_test_times, self.test_times.currentText())
