...
class TestLXStability:
    ...
    def test_lx_boot_check_stability_test(self):
        ...
        # 获取测试点的配置信息
        is_wifi = self.ui_conf_file.get(Config.section_ui_to_background, Config.option_wifi_test)
        is_eth = self.ui_conf_file.get(Config.section_ui_to_background, Config.option_eth_test)
        is_mobile = self.ui_conf_file.get(Config.section_ui_to_background, Config.option_mobile_test)
        is_bt = self.ui_conf_file.get(Config.section_ui_to_background, Config.option_bt_test)
        is_nfc = self.ui_conf_file.get(Config.section_ui_to_background, Config.option_nfc_test)
        ...
        if int(is_eth):
            ...
            if int(is_mobile):
                ...
                if self.device.mobile_is_enable():
                    ...
                    time.sleep(3)
                if self.device.mobile_is_enable():
                    log.error("移动数据模块无法下点，请检查！！！")
                    failed_flag += 1
            try:
                self.device.ping_network(5, 35)
